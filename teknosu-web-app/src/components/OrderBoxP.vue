<template>
    ...
    <div class="card-body">
      ...
      <p class="date">Address: {{order.name}} {{ order.address}} {{ order.city}} {{ order.country}} {{order.zipCode}}</p>
      <p class="date">Payment Id: {{randomNumber()}}</p>
      <p class="card-text"> Total Price: ${{calculatetotalprice(order)}} (Tax: 0%)</p>   
        <div class="card-body">
          <p align="center">
      <button class= "button" @click="toggle()">Download Invoice</button>
    </p>
        ...
        <template #content>
            ...
            <ProductBoxOrderP :productorder="prod" :theorder="order" :theproductimg="imgData(prod)"/>
...
import ProductBoxOrderP from './ProductBoxOrderP.vue'
import {
  VueCollapsiblePanelGroup,
  VueCollapsiblePanel,
} from '@dafcoe/vue-collapsible-panel'
export default {
  components: {ProductList,VueCollapsiblePanelGroup,ProductBoxOrderP,
  VueCollapsiblePanel},
  props: ["order"],
  data() {
    return{
